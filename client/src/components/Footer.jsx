        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-10 mb-16">
          <div className="col-span-2">
            <Link to="/" className="flex items-center gap-4 mb-5 group">
              <div className="relative w-12 h-12 rounded-2xl bg-gradient-to-br from-primary-500 via-coral to-coral-dark flex items-center justify-center shadow-lg shadow-primary-500/30 ring-1 ring-white/10 group-hover:ring-white/20 transition-all">
          ...
          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title}>
              ...
              <ul className="space-y-3">
                {links.map((link) => (
                  <li key={link.name}><Link to={link.href} className="text-white/30 text-sm hover:text-primary-500 transition-colors">{link.name}</Link></li>
                ))}
              </ul>
            </div>
          ))}
