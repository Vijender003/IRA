...
export default function Navbar() {
  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 before:absolute before:inset-0 before:bg-gradient-to-b before:from-primary-400/5 before:to-transparent before:pointer-events-none ${
        scrolled
          ? "bg-surface-950/80 backdrop-blur-xl border-b border-primary-400/15 shadow-2xl shadow-black/30"
          : "bg-surface-950/40 backdrop-blur-md border-b border-primary-400/5"
      }`}
    >
      {/* Animated glow line */}
      <div className="absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-primary-400/60 to-transparent animate-[shimmer_3s_ease-in-out_infinite]" />
      <div className="container-custom flex justify-between items-center px-4 md:px-8 h-20">
        <Link to="/" className="flex items-center gap-4 group">
          <div className="relative w-12 h-12 rounded-2xl bg-gradient-to-br from-primary-400 via-accent-500 to-accent-600 flex items-center justify-center shadow-lg shadow-primary-400/30 group-hover:shadow-primary-400/50 group-hover:scale-105 transition-all duration-300 ring-1 ring-white/10 group-hover:ring-white/20">
            ...
            <div className="absolute -inset-0.5 rounded-2xl bg-gradient-to-br from-primary-400/0 via-accent-500/0 to-accent-500/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-sm" />
          </div>
          ...
          {navLinks.map((link) => {
            ...
            return (
              ...
              >
                {link.name}
                {link.badge && (
                  <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-gradient-to-r from-primary-400 to-accent-500 text-white font-bold animate-pulse tracking-wider">
                    LIVE
                  </span>
                )}
                {isActive && (
                  <motion.div
                    ...
                    className="absolute -bottom-0.5 left-3 right-3 h-0.5 bg-gradient-to-r from-primary-400 to-accent-500 rounded-full"
        ...
        <div className="hidden lg:flex items-center gap-4">
          {isAuthenticated ? (
            ...
            <>
              ...
              <Link to="/register" className="bg-gradient-to-r from-primary-400 to-accent-500 text-white text-sm font-semibold px-6 py-2.5 rounded-xl hover:shadow-[0_0_30px_rgba(34,211,238,0.3)] hover:scale-[1.02] transition-all duration-300">
                Get Started
              </Link>
            </>
          )}
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="lg:hidden bg-surface-950/80 backdrop-blur-xl border-t border-primary-400/10"
          >
              ...
              <div className="border-t border-white/5 pt-3 mt-2 space-y-2">
                {isAuthenticated ? (
                  ...
                  <>
                    ...
                    <Link to="/register" onClick={() => setMobileOpen(false)} className="block bg-gradient-to-r from-primary-400 to-accent-500 text-white text-center font-semibold py-3 px-4 rounded-xl">Get Started</Link>
                  </>
                )}
              </div>
            </div>
